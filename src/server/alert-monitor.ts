...
import { fetchOkxTickers, type OkxTicker } from './okx-tickers';

export type TickerFetcher = () => Promise<OkxTicker[]>;

export type AlertMonitorOptions = {
  store: AlertStore;
  notifier: Notifier;
  fetchTickers?: TickerFetcher;
  intervalMs?: number;
  logger?: (message: string) => void;
};

/**
 * Periodically checks active price alerts against the latest OKX prices and
 * pushes a notification for each alert whose condition is met. Alerts fire at
 * most once: the store marks the alert triggered before the notification is
 * sent, so a failure between mark and send cannot cause a duplicate push.
 *
 * A tick that is still running when the next interval fires is skipped, so a
 * slow OKX fetch never queues up overlapping checks.
 */
export class AlertMonitor {
  private readonly store: AlertStore;
  private readonly notifier: Notifier;
  private readonly fetchTickers: TickerFetcher;
  ...
  constructor(options: AlertMonitorOptions) {
    ...
    this.fetchTickers = options.fetchTickers ?? (() => fetchOkxTickers());
