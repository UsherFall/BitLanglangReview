...
- `TradeChart` has an all open/close markers mode: a toolbar toggle fetches `/api/trades?instrument=...` (unfiltered), computes `allTradeMarkers` with the active Trade highlighted, and hides the single-trade Eye button while active. Keep the marker list in the pure helper `src/ui/trade-markers.ts` and update both series whitespace and markers when the all-trade set changes. Marker styling uses the Gate.io-style B/S square markers: buy points show a green `B` marker below the bar, sell points show a red `S` marker above the bar, each with the price text beside it. Markers are rendered through lightweight-charts' built-in canvas marker API rather than a React SVG overlay, so chart dragging stays smooth; active markers are larger and non-active markers are smaller.
